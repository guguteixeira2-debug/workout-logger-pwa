 ...
 * É a MESMA URL usada no index.html da casca antiga — só que agora chamada
 * via fetch() em vez de carregada num iframe.
 */
...
// Requisições GET: leituras (listar rotinas, exercícios, histórico, calendário).
// Funcionam cross-origin sem configuração extra no Apps Script.
...
// Requisições POST: escritas (salvar série/exercício, finalizar treino, cardio).
// Content-Type "text/plain" é PROPOSITAL — evita o preflight OPTIONS que o
// Apps Script não sabe responder. Não troque para "application/json".
...
var app = (function () {
  ...
  function iniciarTimerDescanso() {
    ...
    estado.timerDescansoId = setInterval(function () {
      ...
      var mStr = mins < 10 ? "0" + mins : mins;
      var sStr = segs < 10 ? "0" + segs : segs;
      document.getElementById('tempo-descanso-display').innerText = mStr + ":" + sStr;
